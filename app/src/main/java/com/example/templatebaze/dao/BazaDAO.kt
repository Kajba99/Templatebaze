...
import com.example.templatebaze.baza.*
...
class BazaDAO( val context : Context) : DAO {
    ...
    override fun insert( s: Student ): Boolean {
        ...
        values.put( COLUMN_GLAZBA,          s.glazba )
        try {
    ...
    override fun getAll( ): List< Student > {
        ...
        if( cursor!!.moveToFirst( ) ) {
            while( !cursor.isAfterLast ) {
                ...
                val glazba  =     cursor.getString( cursor.getColumnIndex( COLUMN_GLAZBA ) )
                ...
                studenti.add( Student( id, ime, rodendan, godina, spol, glazba ) )
